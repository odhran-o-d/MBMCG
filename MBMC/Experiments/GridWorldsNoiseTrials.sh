...
rsync --include="tmp.mat" --exclude="*.mat" ../* workdir/
cp -a ../MBMC_master.m ../setDefaultSwitches.m ../ChunkPath_dakota.m ../createAgent.m ../activateChunkCells.m ../ChunkController.m ../ChunkLearner.m ../ChunkMaze_World.m workdir/
#cp -R ../Diagnostics ../"Simulation Functions" ../"Support Functions" workdir/
rsync -a ../Diagnostics ../Experiments ../"Simulation Functions" ../"Support Functions" ../Walls ../Agents workdir/ --exclude workdir/ # performs a similar function to "cp" but excludes workdir, preventing recursive loop
...
echo matlab -nojvm -nodesktop -nodisplay -nosplash -r "ChunkPath_dakota(); trialSuccessRate('params.in', 'results.out'); exit"

matlab -nojvm -nodesktop -nodisplay -nosplash -r "ChunkPath_dakota(); trialSuccessRate('params.in', 'results.out'); exit"
