function hc = compass_copy(color, varargin)
...
%set(cax, 'Color') = colour;

%xx = [0 1 .8 1 .8].';
%yy = [0 0 .08 0 -.08].';
arrow_size = 0.3;
xx = [0 1 (.8 - arrow_size) 1 (.8 - arrow_size)].';
yy = [0 0 (.08 + arrow_size) 0 (-.08 - arrow_size)].';
arrow = xx + yy.*sqrt(-1);
...
if isempty(s),
    h = polar_copy(color, cax,th,r);
    co = get(cax,'colororder');
    set(h,'color',co(1,:))
else
    h = polar_copy(cax,th,r,s);
end
