...
cp -a MBMC_dakota.m ../MBMC_master.m ../ChunkPath.m ../createAgent.m ../activateChunkCells.m ../ChunkController.m ../ChunkLearner.m ../ChunkMaze_World.m workdir/
#cp -R ../Diagnostics ../"Simulation Functions" ../"Support Functions" workdir/
rsync -a ../Diagnostics ../"Simulation Functions" ../"Support Functions" workdir/ --exclude workdir/ # performs a similar function to "cp" but excludes workdir, preventing recursive loop
...
echo matlab -nojvm -nodesktop -nodisplay -nosplash -r "MBMC_dakota('params.in', 'results.out'); exit"

matlab -nojvm -nodesktop -nodisplay -nosplash -r "MBMC_dakota('params.in', 'results.out'); exit"
